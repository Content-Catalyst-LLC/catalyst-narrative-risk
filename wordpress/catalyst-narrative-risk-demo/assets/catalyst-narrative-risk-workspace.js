(function () {
  ...
  if (!engine) return;
  const STORAGE_KEY = 'catalyst_narrative_risk_workspace_v1_10_0';
  ...
  function normalizeCaseItem(item) {
    ...
    item.retention_assessments = Array.isArray(item.retention_assessments) ? item.retention_assessments : [];
  ...
  function bundle(caseItem) {
    const base = {
      bundle_type: 'catalyst_narrative_risk_case_bundle', bundle_version: '2.0.0', exported_at: now(),
      ...
      retention_assessments: caseItem.retention_assessments
    };
  ...
  function caseCard(root, item) {
    ...
    const exportButton = document.createElement('button'); exportButton.type = 'button'; exportButton.textContent = 'Export'; exportButton.addEventListener('click', function () { download('narrative-risk-case-v2.0.0.json', bundle(item)); });
  ...
  function createOrRevise(root, form) {
    ...
    if (!item) {
      item = {
        ...
        publication_briefings: [], publication_packages: [], public_embeds: [], platform_handoffs: [], retention_assessments: []
      };
  ...
  function monitoringSnapshot(item, trigger) {
    ...
    const final = latestFinal(item); const snapshot = {
      snapshot_id: uuid(), snapshot_version: '2.0.0', case_id: item.case.case_id, revision_id: revision.revision_id,
  ...
  function importBundle(root, file) {
    const reader = new FileReader(); reader.onload = function () {
      try {
        const data = JSON.parse(reader.result); if (data.bundle_type !== 'catalyst_narrative_risk_case_bundle' || data.bundle_version !== '2.0.0') throw new Error('Not a v2.0.0 narrative-risk case bundle.');
