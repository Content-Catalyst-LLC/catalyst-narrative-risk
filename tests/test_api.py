...
def test_api_returns_canonical_record():
    ...
    assert record["contract"]["contract_version"] == "2.0.0"
    assert record["method_snapshot"]["method_version"] == "2.0.0"
    assert record["identifiers"]["ledger_schema_id"].endswith("/2.0.0")
...
def test_api_exposes_contract_vocabularies_and_method_snapshot():
    ...
    assert vocabularies["vocabulary_version"] == "2.0.0"
    assert "evidence_relation_type" in vocabularies["vocabularies"]
    assert method["method_snapshot"]["method_version"] == "2.0.0"
    assert method["method_snapshot"]["ledger_policy"]["policy_version"] == "2.0.0"
...
def test_api_analyzes_evidence_ledger():
    ...
    assert analysis["evidence_ledger"]["derived_scoring_inputs"] == {
        ...
        "source_count": 2,
        "basis": "Derived from evidence relationships linked to the primary claim using the embedded v2.0.0 ledger policy.",
    }
...
def test_api_rejects_missing_claim_invalid_vocabulary_and_non_json_body():
    ...
    assert non_json.get_json()["message"] == "Content-Type: application/json is required"
...
def test_api_saved_views_archive_restore_and_workspace_health(tmp_path):
    ...
    assert health["workspace_version"] == "2.0.0"
...
def test_api_analyzes_narrative_map():
    ...
    assert payload["narrative_map"]["map_version"] == "2.0.0"
