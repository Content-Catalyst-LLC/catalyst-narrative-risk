...
def test_api_returns_canonical_record():
    ...
    assert record["contract"]["contract_version"] == "1.7.0"
    assert record["method_snapshot"]["method_version"] == "1.7.0"
    assert record["identifiers"]["ledger_schema_id"].endswith("/1.7.0")
...
def test_api_exposes_contract_vocabularies_and_method_snapshot():
    ...
    assert contract["ledger_schema_id"].endswith("/1.7.0")
    assert vocabularies["vocabulary_version"] == "1.7.0"
    assert "evidence_relation_type" in vocabularies["vocabularies"]
    assert method["method_snapshot"]["method_version"] == "1.7.0"
    assert method["method_snapshot"]["ledger_policy"]["policy_version"] == "1.7.0"
...
def test_api_analyzes_evidence_ledger():
    ...
    assert analysis["evidence_ledger"]["derived_scoring_inputs"] == {
        ...
        "source_count": 2,
        "basis": "Derived from evidence relationships linked to the primary claim using the embedded v1.7.0 ledger policy.",
    }
...
def test_api_migrates_v1_1_0_record():
    ...
    assert migrated["contract"]["contract_version"] == "1.7.0"
...
def test_api_analyzes_narrative_map():
    ...
    assert payload["narrative_map"]["map_version"] == "1.7.0"
