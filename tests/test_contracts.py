...
def test_contract_registry_points_to_v1_3_0_assets():
    ...
    assert contract["compatibility"]["migrates_from"] == ["1.0.1", "1.1.0", "1.2.0", "1.3.0", "1.4.0"]
    assert contract["ledger_schema_id"].endswith("/1.5.0")
    assert contract["narrative_map_schema_id"].endswith("/1.5.0")
    assert contract["knowledge_library_handoff_schema_id"].endswith("/1.5.0")
    assert contract["catalyst_data_handoff_schema_id"].endswith("/1.5.0")
...
def test_method_and_vocabularies_are_version_aligned():
    ...
    assert method["controlled_vocabulary_version"] == vocabs["vocabulary_version"] == "1.5.0"
    assert method["ledger_policy"]["policy_version"] == "1.5.0"
    assert method["narrative_map_policy"]["policy_version"] == "1.5.0"
